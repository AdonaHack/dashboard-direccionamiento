...
let currentMarker;
let todasLasNaps = [];
let datosCargadosCompletos = null;
let cacheCoordenadas = {};

function validarCredenciales() {
    const userIngresado = document.getElementById('login-user').value.trim();
    const passIngresado = document.getElementById('login-pass').value.trim();
    const errorMsg = document.getElementById('login-error');

    if (!userIngresado || !passIngresado) {
        errorMsg.textContent = 'Por favor completa todos los campos.';
        return;
    }

    errorMsg.textContent = 'Validando acceso...';

    // Usamos el nombre explícito de la hoja 'CREDENCIALES' para evitar confusiones de GID
    const scriptUrl = `https://docs.google.com/spreadsheets/d/${SPREADSHEET_ID}/gviz/tq?sheet=CREDENCIALES&tqx=responseHandler:procesarRespuestaLogin&t=${new Date().getTime()}`;

    const scriptAnterior = document.getElementById('auth-script');
    if (scriptAnterior) scriptAnterior.remove();

    window.procesarRespuestaLogin = function (datos) {
        try {
            const filas = datos.table.rows;
            let accesoPermitido = false;

            console.log("--- CREDENCIALES LEÍDAS DE LA HOJA ---");
            for (let i = 0; i < filas.length; i++) {
                const u = filas[i].c && filas[i].c[0] && filas[i].c[0].v !== null ? String(filas[i].c[0].v).trim() : '';
                const p = filas[i].c && filas[i].c[1] && filas[i].c[1].v !== null ? String(filas[i].c[1].v).trim() : '';

                console.log(`Fila ${i}: Usuario = "${u}" | Password = "${p}"`);

                if (u.toLowerCase() === 'usuario' || u.toLowerCase() === 'user') continue;

                if (u.toLowerCase() === userIngresado.toLowerCase() && p === passIngresado) {
            ...
            console.error('Error al procesar credenciales:', e);
            errorMsg.textContent = 'Error al verificar las credenciales.';
        }
    };
    ...
    script.onerror = () => {
        errorMsg.textContent = 'Error de conexión o permisos en la hoja.';
    };
...
function cambiarZona() {
    ...
    actualizarSelectDistritos();
    filtrarDatos();
}

function cargarDatos() {
    if (datosCargadosCompletos) {
        procesarDatosNaps(datosCargadosCompletos);
        return;
    }

    document.getElementById('tabla-naps').innerHTML = '<tr><td colspan="5" style="text-align:center;">Cargando datos del sistema...</td></tr>';

    const gidSeleccionado = document.getElementById('filtro-zona').value;
    const scriptUrl = `https://docs.google.com/spreadsheets/d/${SPREADSHEET_ID}/gviz/tq?gid=${gidSeleccionado}&tqx=responseHandler:procesarRespuestaGoogle&t=${new Date().getTime()}`;
...
function procesarDatosNaps(datos) {
    ...
    for (let i = 4; i < filas.length; i++) {
        const zona = getVal(i, 0).toUpperCase();
        const distrito = getVal(i, 1).toUpperCase();
        ...
        if (!napCode && !direccion && !distrito && !zona) continue;

        if (napCode && zona && distrito) {
            todasLasNaps.push({
                zona,
                distrito,
                direccion,
...
function actualizarSelectDistritos() {
    const selectZona = document.getElementById('filtro-zona');
    const zonaSeleccionada = selectZona.options[selectZona.selectedIndex].text.toUpperCase();
    const selectDistrito = document.getElementById('filtro-distrito');

    let distritosDeZona = new Set();
    todasLasNaps.forEach(item => {
        if (item.zona === zonaSeleccionada) {
            distritosDeZona.add(item.distrito);
        }
    });

    selectDistrito.innerHTML = '<option value="TODOS">Todos los Distritos</option>';
    Array.from(distritosDeZona).sort().forEach(d => {
...
function filtrarDatos() {
    ...
    document.getElementById('kpi-zona').textContent = zonaSeleccionada;
    document.getElementById('kpi-distrito').textContent = distritoSeleccionado;

    const tbodyNaps = document.getElementById('tabla-naps');
    tbodyNaps.innerHTML = '';

    let filtradas = todasLasNaps.filter(n => n.zona === zonaSeleccionada);

    if (distritoSeleccionado !== 'TODOS') {
        filtradas = filtradas.filter(n => n.distrito.toUpperCase() === distritoSeleccionado.toUpperCase());
    }
    ...
    if (filtradas.length === 0) {
        tbodyNaps.innerHTML = '<tr><td colspan="5" style="text-align:center;">No se encontraron registros para esta selección.</td></tr>';
